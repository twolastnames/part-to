...
export interface DefinitionGet200Body {
  duration: Duration;
  description: string;
...
export interface DefinitionGetArguments {
  task: TaskDefinitionId;
}

interface ExternalMappers {
  [status: string]: (arg: Wire200Body) => DefinitionGet200Body;

  200: (arg: Wire200Body) => DefinitionGet200Body;
}

export type DefinitionGetResult = Result<DefinitionGet200Body>;

export const useDefinitionGet: (
  args: DefinitionGetArguments,
) => DefinitionGetResult = ({ task }) =>
  useGet<Wire200Body, DefinitionGet200Body, ExternalMappers>(
    "/api/task/",
    [
      {
        name: "task",
