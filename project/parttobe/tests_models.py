...
from parttobe.models import start_run
...
class PartToRunTestClass(TestCase):
    def setUp(self):
        get_task_definitions.cache_clear()

    @classmethod
    def setUpClass(cls):
        super(PartToRunTestClass, cls).setUpClass()
        part_to1 = PartTo.objects.create(name="My Part To One")
        PartToRunTestClass.part_to1 = part_to1
        PartToRunTestClass.task1_1 = TaskDefinition.objects.create(
            initial_duration=datetime.timedelta(minutes=1),
            part_to=part_to1,
            description="do task1-1",
        )
        PartToRunTestClass.duty1_1 = TaskDefinition.objects.create(
            initial_duration=datetime.timedelta(seconds=20),
            part_to=part_to1,
            description="do duty1-1",
            depended=PartToRunTestClass.task1_1,
            engagement=10,
        )
        PartToRunTestClass.task1_2 = TaskDefinition.objects.create(
            initial_duration=datetime.timedelta(minutes=2),
            part_to=part_to1,
            depended=PartToRunTestClass.duty1_1,
            description="do task1-2",
        )
        part_to2 = PartTo.objects.create(name="My Part To Two")
        PartToRunTestClass.part_to2 = part_to2
        PartToRunTestClass.duty2_1 = TaskDefinition.objects.create(
            initial_duration=datetime.timedelta(seconds=30),
            part_to=part_to2,
            description="do duty2-1",
            engagement=20,
        )
        PartToRunTestClass.task2_1 = TaskDefinition.objects.create(
            initial_duration=datetime.timedelta(minutes=4),
            part_to=part_to2,
            depended=PartToRunTestClass.duty2_1,
            description="do task2-1",
        )
        PartToRunTestClass.task2_2 = TaskDefinition.objects.create(
            initial_duration=datetime.timedelta(minutes=2),
            part_to=part_to2,
            description="do task2-2",
            depended=PartToRunTestClass.task2_1,
        )
        PartToRunTestClass.runner = start_run([part_to1, part_to2])
    ...
    def test_no_task(self):
        ...
        self.assertRaises(StopIteration, get_last)
    ...
    def test_first_task_has_start(self):
        first = next(PartToRunTestClass.runner)
        self.assertEqual(bool(first.started), True)
