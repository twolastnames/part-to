...
image: $(VENV)
	version=$(shell python bin/version.py)
	sudo bash -c "$(WITH_VENV) docker build -t twolastnames/part-to:$(version) ."
	sudo bash -c "$(WITH_VENV) docker push twolastnames/part-to:$(version)"
